            ...
            VStack(spacing: 0) {
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        Spacer(minLength: 4)
                        Button {
                            if let transcript = store.entries.first(where: { $0.kind == .transcript }) {
...
private struct ClipboardCard: View {
    ...
    var body: some View {
        ...
        .onHover { hovering = $0 }
        .contextMenu {
            Button(entry.isFavorite ? "Убрать из избранного" : "В избранное", action: favorite)
            Divider()
            Button("Удалить", role: .destructive, action: delete)
        }
        .accessibilityLabel("\(entry.kind.title), \(entry.sourceApp)")
