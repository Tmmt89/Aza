...
- **Тесты**: 17 XCTest в цели AzaTests, `xcodebuild test` в CI.
...
- **Расписания ДУМ** — механизм чтения готов, данные собраны (63 города:
  ДУМ РТ 44, ЦДУМ 18, РДУМ ЧО 1), каталог установлен локально в
  Application Support/Aza/prayer-schedules и намеренно НЕ в репозитории.
  Осталось письменное разрешение на распространение — условия и список
  запросов в [docs/PLAN-prayer-schedules.md](docs/PLAN-prayer-schedules.md).
  Перед каждым коммитом, который трогает макеты, спецификацию или тесты
  с временами, гонять `Tools/check-schedule-leak.sh` (код 1 — данные
  каталога попали в трекаемые файлы, 2 — каталогов нет и проверка не
  выполнена). Одиночное значение скрипт не ловит — смотреть глазами.
