...
### P1 — Этап 2 (буфер): ЗАКРЫТ 27.08
Всё из docs/PLAN-stage2-clipboard.md реализовано: retention, все виды
записей (text/rtf/image/files/link, blob-хранилище изображений),
миниатюры + «Показать целиком», блокировка Mac (§8.9), пользовательские
исключения приложений, массовое удаление найденного с пакетной отменой
(§8.7). Отложено до island-UI: клавиатурные Delete/⌘A по выделению.
Keychain-диалоги при пересборках побеждены: штатная подпись
«Aza Development» + «Разрешить всегда» (см. §7).
